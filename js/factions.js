...
const FACTIONS = {
  bug: {
    ...
    desc: ['开战：全体友方 +1 生命', '开战：全体友方 +3 生命']
  },
  aqua: {
    cn: '水生', icon: '🐟', tiers: [2, 4],
    desc: ['开战：最前排 +2/+2', '开战：全体友方 +2/+2']
  },
  bird: {
    cn: '飞禽', icon: '🐦', tiers: [2, 3],
    desc: ['每回合开始：随机 1 个友方 +1/+1', '每回合开始：全体友方 +1/+1']
  },
  beast: {
    cn: '巨兽', icon: '🐘', tiers: [2, 4],
    desc: ['开战：全体友方 +1 攻击', '开战：全体友方 +3 攻击']
  },
  critter: {
  ...
  reptile: {
    ...
    desc: ['开战：全体友方获得辣椒（溅射 5）', '开战：全体友方 +3/+3']
  }
};
...
function applySynergyBattleStart(game, myTeam) {
  ...
  // 🐜 虫豸：全体加生命
  const bug = lvl('bug');
  if (bug >= 1) for (const p of myTeam) p.hp += (bug >= 2 ? 3 : 1);

  // 🐟 水生：2 档强化全体，1 档只强化最前排
  const aqua = lvl('aqua');
  if (aqua >= 2) { for (const p of myTeam) { p.atk += 2; p.hp += 2; } }
  else if (aqua === 1 && myTeam[0]) { myTeam[0].atk += 2; myTeam[0].hp += 2; }

  // 🐘 巨兽：全体加攻击
  const beast = lvl('beast');
  if (beast >= 1) for (const p of myTeam) p.atk += (beast >= 2 ? 3 : 1);
  ...
  if (reptile >= 2) { for (const p of myTeam) { p.atk += 3; p.hp += 3; } }
  else if (reptile === 1) {
